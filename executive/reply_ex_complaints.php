...
?>
  <tr>
    <td width="20%" height="36"><a href="executivehome.php?view=ex_complaints"><input type="button" value="Back"></a></td></tr>
...
<tr>
 ...
 {
 echo "<center><font color=#FF00FF>The complaint is solved</font></center>";
 }
 else
 {
 ...
 ?>           
   ...
   <select name="ex" style="width:200px" ><option value="">---SELECT---</option>
   
   <option value="ex_complet">Completed</option>
   <option value="ex_incomplete">Not Working</option>
   
   
   
  </select>
  
&nbsp;<input type="submit" value="Completed" name="assign">
    <?php
 }
...
?>
 ...
 {
	 	$ex=$_POST['ex']; 
	 	$s231=mysql_query("select * from  complaints where co_id='$comp_id' and c_status='$ex'") or die(mysql_error());
	if((mysql_num_rows($s231))>0)
	{
		echo"<script>alert('Already Completed')</script>";	

	}

	mysql_query("update complaints set c_status='$ex' where co_id='$comp_id'") or die(mysql_error());
...
else if($_POST['send'])
 {
	...
	echo"<script>window.location='executivehome.php?view=ex_sendreport&co_id=$comp_id'</script>";
