...
$s="select * from login where username='$user'";
	...
	if($opass==$val['password'])
	{
			...
			$s11="update service_mgr set mgr_pwd='$npass' where mgr_una='$user'";
			mysql_query($s11);
			echo "<script> alert('Password Changed!') </script>";
			echo "<script> window.location='servicemgrhome.php'</script>";
		
		}
	else
	{
		echo "<script> alert('Check your Password!') </script>";
		echo "<script> window.location='servicemgrhome.php?view=m_changepass'</script>";
...
<form id="form1" name="form1" method="post" action="">
 <center> <table width="518" height="342" border="0">
  <tr>
      ...
      <td width="238"><input type="text" readonly="readonly" name="textfield" id="user" value="<?php echo $user;?>"/></td>
    </tr>
    <tr>
...
<script  language="javascript" type="text/javascript">
 ...
 frmvalidator.addValidation("opass","req","Enter the Old Pasword!");
  var frmvalidator = new Validator("form1");
 frmvalidator.addValidation("npass","req","Enter the New Password!");

  var frmvalidator = new Validator("form1");
 frmvalidator.addValidation("cpass","req","Please Confirm your Password!");
